    ...
    <header className="sticky top-0 z-30 backdrop-blur bg-ink/80 border-b border-white/5">
      <nav className="max-w-6xl mx-auto px-4 py-3 flex items-center gap-3">
        <Link to="/" className="font-bold text-lg tracking-tight flex items-center gap-2 shrink-0">
          ...
          <span className="bg-gradient-to-r from-grape-soft to-sun bg-clip-text text-transparent hidden sm:inline">Agent Lab</span>
        </Link>

        {onChapter && (
          <>
            {/* Compact stepper — small numbered dots, current one highlighted */}
            <div className="hidden md:flex items-center gap-1 mx-2">
              {prev && (
                <Link to={prev.path} title={`Prev · ${prev.title}`} className="px-2 py-1 rounded text-paper/50 hover:text-paper text-sm">←</Link>
              )}
              ...
              {next && (
                <Link to={next.path} title={`Next · ${next.title}`} className="px-2 py-1 rounded text-paper/50 hover:text-paper text-sm">→</Link>
              )}
            </div>

            <div className="hidden lg:flex items-center text-sm text-paper/70 px-2 truncate">
              <span className="text-paper/40 font-mono text-xs mr-2">{current!.n}</span>
              <span className="mr-1.5">{current!.emoji}</span>
              <span className={`font-semibold ${current!.accentText} truncate max-w-[200px]`}>{current!.title}</span>
            </div>
          </>
        )}
